...
void initfase2(void)
{
    ...
    PhysicsBody basePhys = CreatePhysicsBodyRectangle((Vector2){0, 50}, 1280, 100, 10);
    basePhys->enabled = false;
...
void Refresh()
{
    ...
    if (!portalativo && !lost && !gotCristal && !victoryFase2)
    {
        ...
        if (frames % 100 == 0)
        {
            ...
            if (repetido)
                DestroyPhysicsBody(tiles[i].body);
        ...
        for (int z = 0; z < 8; z++)
        {
            ...
            if (tiles[z].isMoving)
            {
                ...
                if (tiles[z].body->position.x >= 255)
                    tiles[z].movementspeed = -tiles[z].movementspeed;
                if (tiles[z].body->position.x <= -255)
                    tiles[z].movementspeed = -tiles[z].movementspeed;
        ...
        if (cameraJumper.target.y < (-200))
            paralaxFase2 -= 0.8;
    }
    if (player.body->position.y >= cameraJumper.target.y + AlturaDaTela * 1.5)
        lost = true;
    ...
    if (!portalativo)
    {
        ...
        {
            player.body->velocity.x = VELOCITY * 1.1;
            player.direita = true;
        }
    ...
    else if (player.body->velocity.x == 0 && !player.direita)
        player.rect.y = 3 * player.txt.height / 4;
    else
    {
        if (player.direita)
            player.rect.y = 0;
        else
            player.rect.y = player.txt.height / 4;
    }
    ...
    if (CheckCollisionPointRec(player.body->position, cristalrect))
        gotCristal = true;
}
...
void Desenhar()
{
    ...
    if (portalativo)
        DrawTextureRec(portalFase2, portalrect, (Vector2){player.body->position.x - 120, player.body->position.y - 100}, WHITE);
    if (gotCristal)
        DrawTextureRec(portalFase2, portalrect, (Vector2){-100, cameraJumper.target.y - 450}, WHITE);
