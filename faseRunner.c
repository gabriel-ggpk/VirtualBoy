...
float frames_countdown = 0;
float walking = 0;
float frame_spawn = 0;
float frame_portal = 0;
float frame_coin = 0;
float exitrun = 0;
...
bool dead = false;
bool down = false;
bool portalframe = true;
bool coindown = false;
bool win = false;
bool despawn = false;
bool despawnport = false;
...
void contador()
{
    ...
    if (countdown < 1 && walking >= 0.005 && !dead)
    {
        ...
        {
            if (timer > 41)
                personagem.body->position.x += 5.0f;
            else if (timer > 21)
                personagem.body->position.x += 6.0f;
            else
                personagem.body->position.x += 7.0f;
        }
        for (int i = 0; i < 3; i++)
...
void movimentacao()
{
    ...
    {
        if (!personagem.body->isGrounded)
            personagem.body->velocity.y = 0.8f;
        else
        {
...
void colisao()
{
    ...
    if (timer > 0 && !dead)
    {
        ...
        {
            if (!dead)
                dead = CheckCollisionRecs(personagem.collision, rock[i].collision);
            if (!dead)
                dead = CheckCollisionRecs(personagem.collision, bird[i].collision);
        }
        if (dead)
        {
            PlaySound(death);
    ...
    else if (timer < 2 && !win)
    {
        ...
        {
            coindown = true;
            if (!CheckCollisionRecs(personagem.collision, coin.collision))
                coin.position.y += 1.2f;
            else
            {
                StopMusicStream(music);
                win = true;
...
void desenho()
{
    if (dead)
        frameline = 4, limit = 2;
    else if (personagem.body->orient != 0)
        frameline = 6, limit = 6;
    else if ((timer > 1 && IsKeyPressed(KEY_UP)) || !personagem.body->isGrounded)
        frameline = 10, limit = 11;
    else if (countdown < 1 && timer > -5 && !coindown)
        frameline = 0, limit = 0;
    else
        frameline = 2, limit = 1;
    ...
    {
        if (timer > 0)
            DrawText(TextFormat("Time: %d", timer), 10, 10, 20, RAYWHITE);
        else
            DrawText("Time: 0", 10, 10, 20, RAYWHITE);
    }
...
int faseRunner(void)
{
    ...
    do
    {
        while (retornoRunner == 0)
        {
            ...
            else if (win && exitrun >= 2)
                retornoRunner = 2;

            else if (dead && IsKeyPressed(KEY_R))
                retornoRunner = -1;
